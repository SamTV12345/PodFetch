...
#[put("/settings")]
pub async fn update_settings(db: Data<Mutex<DB>>, settings: web::Json<Setting>) -> impl
Responder {
    let mut db = db.lock().expect("Error acquiring db lock");
...
pub async fn run_cleanup(pdservice: Data<Mutex<PodcastEpisodeService>>, db:Data<Mutex<DB>>) -> impl
Responder {
    let settings = db.lock().expect("Error acquiring db lock").get_settings();
    match settings {
        Some(settings) => {
            pdservice.lock().expect("Error acquiring db lock")
