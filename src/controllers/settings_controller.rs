...
web::Json<Setting>, requester: Option<web::ReqData<User>>,conn:Data<DbPool>) -> Result<HttpResponse, CustomError> {
    ...
    let settings = settings_service.update_settings(settings.into_inner(),conn.get().map_err(map_r2d2_error)?.deref_mut())?;
...
pub async fn run_cleanup(
    ...
    let settings = settings_service.lock().ignore_poison().get_settings( conn.get().map_err(map_r2d2_error)?.deref_mut())?;
    match settings {
        Some(settings) => {
            pdservice
                .lock()
                .ignore_poison()
                .cleanup_old_episodes(settings.auto_cleanup_days,conn.get().map_err(map_r2d2_error)?.deref_mut());
...
Data<Mutex<EnvironmentService>>) -> Result<HttpResponse, CustomError>{
    ...
    let podcasts_found = Podcast::get_all_podcasts(conn.get().map_err(map_r2d2_error)?.deref_mut())?;
...
pub async fn update_name(settings_service: Data<Mutex<SettingsService>>,
    ...
    let settings = settings_service.update_name(update_information.into_inner(), conn.get().map_err(map_r2d2_error)?.deref_mut())?;
    Ok(HttpResponse::Ok().json(settings))
}
