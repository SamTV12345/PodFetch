...
pub mod tests {
    ...
    pub async fn create_auth_gpodder(server: &mut TestServerWrapper<'_>, user: &User) {
        ...
        server.test_server.add_header("Authorization", format!("Basic {}", encoded_auth));

        let response = {
            let server_ref = &mut server.test_server;
            server_ref.post(&format!("/api/2/auth/{}/login.json", user.username)).await
        };
        assert_eq!(response.status_code().as_u16(), 200);
        // get devices
        let cookie_binding = response.cookies();
        server.test_server.add_cookie(cookie_binding.get("sessionid").unwrap().clone());
