...
impl TagsPodcast {
    ...
    pub fn delete_tags_by_podcast_id(
        conn: &mut DbConnection,
        ...
        let _ = insert_with_conn!(conn, |conn| diesel::delete(
    ...
    pub fn delete_tag_podcasts(
        conn: &mut DbConnection,
        tag_id_to_delete: &str,
    ) -> Result<(), CustomError> {
        use crate::dbconfig::schema::tags_podcasts::dsl::*;
        use crate::dbconfig::schema::tags_podcasts::table as t_podcasts;
        use diesel::ExpressionMethods;
        ...
        use diesel::RunQueryDsl;
        let _ = insert_with_conn!(conn, |conn| diesel::delete(
            t_podcasts.filter(tag_id.eq(tag_id_to_delete))
        )
        .execute(conn)
        .map_err(map_db_error));
        Ok(())
