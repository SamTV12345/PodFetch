...
use crate::constants::constants::{ITUNES_URL, PodcastType};
...
impl PodcastService {
    ...
    pub async fn find_podcast_on_podindex(&mut self,podcast: &str) -> Value {
        ...
        let result = self.client.get("https://api.podcastindex.org/api/1.0/search/byterm?q=".to_owned() +
            podcast).headers(headers).send().await.unwrap();
    ...
    pub async fn insert_podcast_from_podindex(&mut self, id: i32, lobby: Data<Addr<Lobby>>){
        ...
        let resp = self.client.get("https://api.podcastindex.org/api/1.0/podcasts/byfeedid?id="
            .to_owned() +
            &id.to_string()).headers(self.compute_podindex_header()).send().await.unwrap();

        println!("Result: {:?}", resp);

        let podcast = resp.json::<Value>().await.unwrap();

            self.handle_insert_of_podcast(PodcastInsertModel {
    ...
    pub async fn handle_insert_of_podcast(&mut self, podcast_insert: PodcastInsertModel,
                                          mapping_service: MappingService, lobby: Data<Addr<Lobby>>){
        let  fileservice = FileService::new();
        let mut db = DB::new().unwrap();
        let inserted_podcast = db.add_podcast_to_database(podcast_insert.title, podcast_insert
            .id.to_string(), podcast_insert.feed_url, podcast_insert.image_url.clone());
        FileService::create_podcast_directory_exists(&podcast_insert.id.clone().to_string());
        fileservice.download_podcast_image(&podcast_insert.id.clone().to_string(),
                                           &podcast_insert.image_url.clone().to_string()).await;
        let podcast = db.get_podcast_by_track_id(podcast_insert.id.clone()).unwrap();
        lobby.get_ref()
            .send(
                BroadcastMessage{
    ...
    fn compute_podindex_header(&mut self) -> HeaderMap {
        ...
        let non_hashed_string = self.environment_service.podindex_api_key
            .clone().to_owned()+ &*self.environment_service.podindex_api_secret.clone()+&seconds
            .to_string();
        ...
        headers.insert("X-Auth-Key", HeaderValue::from_str(&self.environment_service
            .podindex_api_key.clone()).unwrap());
        headers.insert("X-Auth-Date", HeaderValue::from_str(&seconds.to_string()).unwrap());
        headers.insert("Authorization", HeaderValue::from_str(&hashed_auth_key).unwrap());
