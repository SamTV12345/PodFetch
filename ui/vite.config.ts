...
function chartingLibrary(): PluginOption {
	return {
		...
		transformIndexHtml: async (html, _ctx) => {
			...
			const configFromServer = page.mainFrame.document
			...
			page2.content = html
			const configNode = page2.mainFrame.document.createElement('span', {
				is: 'span',
			})
			configNode.id = 'config'
			configNode.setAttribute('data-config', configFromServer)
			page2.mainFrame.document
				.getElementsByTagName('body')[0]
				.appendChild(configNode)
