...
export const MopidyIntegration = () => {
    ...
    const onTest = async () => {
        try {
            ...
            // error already surfaced by middleware
        }
    }
                ...
                <div className="flex flex-col gap-3 max-w-md">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            onClick={onTest}
                            disabled={!url || testServer.isPending}
                        >
                ...
                <table className="text-left text-sm w-full">
                    <thead>
                        <tr className="border-b ui-border">
                            ...
                            <th className="px-2 py-3 ui-text">{t('mopidy-server-shared')}</th>
                            {isAdmin && <th className="px-2 py-3 ui-text">{t('actions')}</th>}
                        </tr>
                    </thead>
                    <tbody>
                        {Array.from({ length: 3 }).map((_, index) => (
                            <tr key={index}>
                                <td className="px-2 py-4 ui-text"><LoadingSkeletonSpan height="20px" loading /></td>
                                <td className="px-2 py-4 ui-text"><LoadingSkeletonSpan height="20px" loading /></td>
                                <td className="px-2 py-4 ui-text"><LoadingSkeletonSpan height="20px" loading /></td>
                                {isAdmin && <td className="px-2 py-4"><LoadingSkeletonSpan height="20px" loading width="60px" /></td>}
                            </tr>
                        ))}
                    </tbody>
                </table>
                        ...
                        {servers.map((server) => (
                            <tr key={server.id}>
                                ...
                                <td className="px-2 py-4">
                                    <Chip index={server.kind === 'mopidy_shared' ? 0 : 4}>
                                        {server.kind === 'mopidy_shared' ? t('mopidy-kind-shared') : t('mopidy-kind-personal')}
                                    </Chip>
                                </td>
                                {isAdmin && (
                                    <td className="px-2 py-4">
                                        <button
                                            className="ui-text-accent hover:underline disabled:opacity-50"
