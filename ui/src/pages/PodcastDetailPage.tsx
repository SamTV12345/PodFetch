...
export const PodcastDetailPage = () => {
	...
	const currentPodcastEpisodes = $api.useQuery(
		'get',
	...
	const refreshPodcastEpisodes = $api.useMutation(
		'post',
	...
	const isOverflown = (element: string) => {
		...
		if (foundElement) {
			return (
				...
				foundElement.scrollWidth > foundElement.clientWidth
			)
		}
		...
		<Fragment key={'detail'}>
			<div className="max-w-4xl">
				...
				>
                    ...
                    "
							src={currentPodcast.data.image_url}
							alt=""
						/>
					)}
					...
					>
						...
						<span>
							{currentPodcast.data && data?.role === ADMIN_ROLE && (
								<PodcastSettingsModal podcast={currentPodcast.data} />
							)}
						</span>
					</div>
                    ...
                    "
					>
						<span className="block text-(--fg-secondary-color)">
							{currentPodcast.data?.author}
						</span>
						...
						<span className="grid grid-cols-2 md:grid-cols-3">
							<a
								...
								href={currentPodcast.data?.podfetch_feed}
								target="_blank"
							>
								<span className="material-symbols-outlined cursor-pointer text-(--fg-icon-color) hover:text-(--fg-icon-color-hover)">
									rss_feed
								</span>
							...
							>
								<a
									...
									target="_blank"
									rel="noopener noreferrer"
									href={currentPodcast.data?.rssfeed}
								>
									rss_feed
								</a>
								<span className="text-(--fg-color)">
									{t('original-rss-feed')}
								</span>
							</button>
							<div className="flex gap-4 justify-end">
								<Switcher checked={onlyUnplayed} onChange={setOnlyUnplayed} />
								<span className=" text-(--fg-color) mt-auto">
									{t('unplayed')}
								</span>
							</div>
						</span>
					...
					>
						...
						<Switcher
							checked={currentPodcast.data?.active}
							onChange={() => {
								client
...

				{
					...
					currentPodcast.data?.summary && (
						<div className="relative leading-[1.75] mb-8 text-sm text-(--fg-color)">
							<div
								id="summary"
								className={lineClamp ? 'line-clamp-3' : ''}
								dangerouslySetInnerHTML={removeHTML(
									currentPodcast.data.summary,
								)}
							/>
							{(isOverflown('summary') || lineClamp) && (
								<div
				...
				<div>
					...
					{currentPodcastEpisodes.data?.map((episode, index) => (
						<PodcastDetailItem
							episode={episode}
							currentEpisodes={currentPodcastEpisodes.data ?? []}
