        ...
        <div className="pb-20">
            <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-sm ui-text-muted">{t('stats-subtitle')}</p>
                </div>

                <div className="grid grid-cols-1 xs:grid-cols-2 gap-3">
                    <label className="flex flex-col gap-1 text-xs ui-text-muted">
                        {t('stats-from')}
                        <input
                            type="date"
                            value={from}
                            onChange={(event) => setFrom(event.target.value)}
                            className="ui-date-input rounded-lg border ui-border ui-surface px-3 py-2 text-sm ui-text focus:outline-none focus:ring-2 focus:ui-ring-accent"
                        />
                    </label>
                    <label className="flex flex-col gap-1 text-xs ui-text-muted">
                        {t('stats-to')}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-10">
                <div className="rounded-xl border ui-border p-4 ui-surface">
                    <p className="text-xs ui-text-muted">{t('stats-listened-podcasts')}</p>
                    <p className="mt-2 text-3xl font-semibold ui-text">
                        {statsQuery.data?.listenedPodcasts ?? 0}
                    </p>
                </div>
                <div className="rounded-xl border ui-border p-4 ui-surface">
                    <p className="text-xs ui-text-muted">{t('stats-listened-episodes')}</p>
                    <p className="mt-2 text-3xl font-semibold ui-text">
                        {statsQuery.data?.listenedEpisodes ?? 0}
                    </p>
                </div>
                <div className="rounded-xl border ui-border p-4 ui-surface">
                    <p className="text-xs ui-text-muted">{t('stats-total-listening-time')}</p>
                    <p className="mt-2 text-3xl font-semibold ui-text">
                        {formatDuration(statsQuery.data?.totalListenedSeconds ?? 0)}
                    </p>
                </div>
                <div className="rounded-xl border ui-border p-4 ui-surface">
                    <p className="text-xs ui-text-muted">{t('stats-active-days')}</p>
                    <p className="mt-2 text-3xl font-semibold ui-text">
            ...
            {statsQuery.isError && (
                <div className="mb-6 rounded-lg border ui-border-danger ui-danger-surface px-4 py-3 text-sm ui-text-danger">
                    {t('error-occured')}
                </div>
            )}

            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <div className="rounded-xl border ui-border p-5 ui-surface">
                    <h2 className="mb-4 text-lg font-semibold ui-text">{t('stats-top-podcasts')}</h2>
                    {statsQuery.isLoading && (
                        <p className="text-sm ui-text-muted">{t('loading')}</p>
                    )}
                    {!statsQuery.isLoading && (statsQuery.data?.topPodcasts?.length ?? 0) === 0 && (
                        <p className="text-sm ui-text-muted">{t('stats-no-data')}</p>
                    )}
                    <div className="flex flex-col gap-3">
                        {statsQuery.data?.topPodcasts?.map((podcast) => (
                            <div key={podcast.podcastId}
                                 className="flex items-center gap-3 rounded-lg border ui-border p-3">
                                <img
                                    src={podcast.imageUrl}
                                    alt={podcast.podcastName}
                                    className="h-12 w-12 rounded-md object-cover ui-input-surface"
                                />
                                <div className="min-w-0 flex-1">
                                    <p className="line-clamp-1 text-sm font-medium ui-text">{podcast.podcastName}</p>
                                    <p className="text-xs ui-text-muted">
                                        {t('stats-listened-episodes')}: {podcast.listenedEpisodes}
                                    </p>
                                </div>
                                <p className="text-xs font-medium ui-text-accent">
                ...
                <div className="rounded-xl border ui-border p-5 ui-surface">
                    <h2 className="mb-4 text-lg font-semibold ui-text">{t('stats-weekday-activity')}</h2>
                    {statsQuery.isLoading && (
                        <p className="text-sm ui-text-muted">{t('loading')}</p>
                    )}
                    ...
                    <div className="flex flex-col gap-3">
                        {statsQuery.data?.activeWeekdays?.map((weekday) => (
                            <div key={weekday.dayIndex} className="grid grid-cols-[88px_1fr_auto] items-center gap-3">
                                ...
                                <div className="h-2 rounded-full ui-input-surface overflow-hidden">
                                    <div
                                        className="h-full rounded-full ui-bg-accent"
                                        style={{
                                            width: `${Math.max(4, (weekday.listenedSeconds / maxWeekdaySeconds) * 100)}%`
                                        }}
                                    />
                                </div>
                                <span className="text-xs ui-text">{formatDuration(weekday.listenedSeconds)}</span>
                            </div>
                        ))}
                    </div>
                </div>
