...
export const Timeline = () => {
	...
	const timeline = $api.useQuery('get', '/api/v1/podcasts/timeline', {
		params: {
			query: {
				favoredOnly:
					filter.data === undefined ? false : filter.data.onlyFavored,
		...
		<div>
				...
				<div className="flex flex-row gap-5">
					<div className="flex items-center gap-3">
						...
						<Switcher
							...
							checked={filter.data?.onlyFavored}
							onChange={() => {
								queryClient.setQueryData(
									['get', '/api/v1/podcasts/filter'],
									(oldData: Filter) => ({
										...oldData,
					...
					<div className="flex items-center gap-3">
						...
						<Switcher
							checked={notListened}
							onChange={() => setNotListened(!notListened)}
						/>
					</div>
					<div className="flex items-center gap-3">
						<span className="text-xs text-(--fg-secondary-color)">
							{t('onlyFavoredEpisodes')}
						</span>

						<Switcher
							checked={notFavored}
							onChange={() => setFavored(!notFavored)}
						/>
					</div>
				</div>
			</div>
			...
			<div className="relative grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-x-8 gap-y-12 pl-6">
				{timeline.isLoading || !timeline.data
					? undefined
					: timeline.data.data.map((e, index) => (
							<Fragment key={`${e.podcast_episode.episode_id + index}Parent`}>
								{
									/* Section start */
									index === 0 ||
									formatTime(e.podcast_episode.date_of_recording) !==
										formatTime(
											timeline.data.data[index - 1]?.podcast_episode
												.date_of_recording ?? '',
										) ? (
										<>
											{/* Date */}
								...
								<TimelineEpisode
									podcastHistoryItem={e.history ?? undefined}
									notListened={notListened}
