...
export const Podcasts: FC<PodcastsProps> = ({ onlyFavorites }) => {
	...
	const memorizedSelection = useMemo(() => {
		return JSON.stringify({
			sorting: filters?.data?.filter?.toUpperCase(),
			ascending: filters?.data?.ascending,
		})
	}, [filters])
		...
		<div>
			...
			<div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-4 mb-10">
				<div className="flex gap-2 items-center">
					...
					<div>
						<CustomSelect
							className="bg-mustard-600 text-black"
			...
			<div className="flex flex-col md:flex-row gap-4 mb-10">
				...
				<CustomSelect
					iconName="sort"
					onChange={(v) => {
						const converted = JSON.parse(v) as OrderCriteriaSortingType
						queryClient.setQueryData(['get', '/api/v1/podcasts/filter'], {
							...filters.data,
