		...
		<div>
			<div className="grid grid-cols-1 xs:grid-cols-[1fr_auto] items-center gap-2 xs:gap-6 mb-10 text-(--fg-color)">
				<div className="flex flex-col gap-2 xs:contents mb-4">
					<div>
						<label className="mr-6" htmlFor="auto-cleanup">
							{t('auto-cleanup')}
						</label>
					...
					<Switcher
						checked={settingsModel.data?.autoCleanup}
				...
				<div className="flex flex-col gap-2 xs:contents mb-4">
					<label htmlFor="days-to-keep" className="flex gap-1">
						{t('days-to-keep')}{' '}
						<SettingsInfoIcon
					...
					<CustomInput
						...
						type="number"
						value={settingsModel.data?.autoCleanupDays}
					/>
				</div>

				<div className="flex flex-col gap-2 xs:contents mb-4">
					<label htmlFor="auto-update" className="flex gap-1">
						{t('auto-update')}{' '}
						<SettingsInfoIcon
					...
					<Switcher
						loading={settingsModel.isLoading}
						checked={settingsModel.data?.autoUpdate}
						className="xs:justify-self-end"
						id="auto-update"
						onChange={() => {
							queryClient.setQueryData(
				...
				<div className="flex flex-col gap-2 xs:contents mb-4">
					<label htmlFor="auto-download" className="flex gap-1">
						{t('auto-download')}{' '}
						<SettingsInfoIcon
					...
					<Switcher
						loading={settingsModel.isLoading}
						checked={settingsModel.data?.autoDownload}
						className="xs:justify-self-end"
						id="auto-download"
						onChange={() => {
							queryClient.setQueryData(
				...
				<div className="flex flex-col gap-2 xs:contents mb-4">
					...
					<CustomInput
						...
						type="number"
						value={settingsModel.data?.podcastPrefill}
					/>
				</div>
				<div className="flex flex-col gap-2 xs:contents mb-4">
