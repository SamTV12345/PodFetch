...
export const SideBar  = ()=>{
    ...
    return <aside className={`float-left ${sideBarCollapsed?'hidden': 'col-span-6 md:col-span-1'} z-10 w-full bg-gray-950 flex  border-none sticky`} aria-label="Sidebar">
        <div className="py-4 px-3 bg-gray-950 w-full">
            <div className="text-amber-600 flex text-2xl gap-2"><PodFetchLogo/><h1 className="text-white pt-1">Podfetch</h1></div>
            <ul className="pt-20">
                <SideBarItem highlightPath={'./'} translationkey={t('homepage')} icon={<i className="fa-solid fa-house fa-xl"></i>}/>
                <SideBarItem highlightPath={'podcasts'} translationkey={t('all-subscriptions')} icon={<PodcastIcon/>}/>
                ...
                <SideBarItem className="absolute bottom-0 w-11/12" highlightPath={"settings"} translationkey={t('settings')} icon={<i className="fa-solid fa-gear fa-xl"/> }/>
            </ul>
        </div>
    </aside>
}
