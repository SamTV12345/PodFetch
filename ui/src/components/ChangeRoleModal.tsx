...
export const ChangeRoleModal = () => {
		...
		client
			.PUT('/api/v1/users/{username}/role', {
				...
				body: {
					...
					explicitConsent: selectedUser?.explicitConsent ?? false,
				},
			})
			.then(() => {
	...
	return (
		<Modal
			headerText={t('change-role-user', { name: selectedUser?.username })}
			onCancel={() => {}}
			onAccept={() => {}}
			onDelete={() => {}}
			cancelText="Test"
		...
		>
			{/* Role select */}
			<div className="mb-6">
				<label className="block mb-2 text-sm text-(--fg-color)" htmlFor="role">
					{t('role')}
				</label>
				<CustomSelect
			...
			<div className="flex items-center gap-4 mb-6">
				...
				<Switcher
					checked={selectedUser?.explicitConsent || false}
			...
			<CustomButtonPrimary className="float-right" onClick={changeRole}>
				{t('change-role')}
			</CustomButtonPrimary>
		</Modal>
	)
