...
export const PodcastInfoModal = () => {
	...
	const deleteEpisodeDownloadOnServer = (episodeId: string) => {
		client
			.DELETE('/api/v1/episodes/{id}/download', {
				params: {
					path: {
						id: episodeId,
					},
	...
	return createPortal(
		...
		>
			...
			>
				...
				<div className="mb-4">
					<Heading2 className="inline align-middle mr-2 break-all">
						{selectedPodcastEpisode?.name || ''}
					</Heading2>

					<span
						className={`material-symbols-outlined align-middle ${selectedPodcastEpisode ? 'cursor-pointer text-(--fg-icon-color) hover:text-(--fg-icon-color-hover)' : 'text-stone-300'}`}
					...
					{selectedPodcastEpisode?.status && (
						<span
							onClick={() =>
								deleteEpisodeDownloadOnServer(
									selectedPodcastEpisode?.episode_id,
								)
							}
							className="material-symbols-outlined align-middle cursor-pointer text-(--danger-fg-color) hover:text-(--danger-fg-color-hover)"
