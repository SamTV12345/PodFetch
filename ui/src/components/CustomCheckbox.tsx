...
export const CustomCheckbox: FC<CustomCheckboxProps> = ({ className = '', id, name, onChange, value }) => {
    return (
        <Checkbox.Root checked={value} className={`align-middle bg-[--input-bg-color] data-[state=checked]:bg-[--accent-color] h-6 w-6 rounded ${className}`} id={id} onCheckedChange={onChange} name={name}>
            <Checkbox.Indicator>
