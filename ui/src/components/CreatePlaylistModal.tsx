...
export const CreatePlaylistModal = () => {
	...
	return createPortal(
		<div
			...
			className={`grid place-items-center fixed inset-0 bg-[rgba(0,0,0,0.5)] backdrop-blur-sm overflow-x-hidden overflow-y-auto z-30 ${playListOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
			tabIndex={-1}
		>
			{/* Modal */}
			<div
				className="relative bg-(--bg-color) text-(--fg-color) max-w-5xl md:w-[50%] p-8 rounded-2xl shadow-[0_4px_16px_rgba(0,0,0,0.2)]"
			...
			>
				...
				<button
					...
					onClick={() => setCreatePlaylistOpen(false)}
				>
					<span className="material-symbols-outlined text-stone-400 hover:text-stone-600">
						close
					</span>
				...
				>
					<div className="mt-5 mb-5 ">
						<Heading2 className="mb-4 text-(--fg-color)">
							{t('add-playlist')}
						</Heading2>
					</div>
					...
					{/* Playlist items */}
					{stage === 1 && <PlaylistSearchEpisode />}

					{/* Buttons for skipping to next step*/}
					<div className="flex">
						<button type="button">
							<span
								className={`material-symbols-outlined ${stage === 0 && 'opacity-60'} text-mustard-600`}
								onClick={() => {
						...
						>
							<span
								className={`material-symbols-outlined ${stage === 2 && 'opacity-60'}  text-mustard-600`}
							>
								arrow_forward
							</span>
						</button>
