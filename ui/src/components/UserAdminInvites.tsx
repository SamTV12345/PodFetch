...
export const UserAdminInvites = () => {
	...
	const [selectedInviteType, setSelectedInviteType] =
		useState<InviteTypeSelection>(InviteTypeSelection.all)
							...
							<tr className="border-b border-(--border-color)" key={i.id}>
								<td className="pr-2 py-4">
									<button
										...
										onClick={() => {
											client
												.GET('/api/v1/invites/{invite_id}/link', {
													parseAs: 'text',
													params: {
														path: {
															invite_id: i.id,
									...
									>
										{i.id}
										<span className="material-symbols-outlined align-middle ml-1">
											link
										</span>
									</button>
								</td>
								<td className="px-2 py-4">{t(i.role)}</td>
								<td className="px-2 py-4">{formatTime(i.createdAt)}</td>
								<td className="px-2 py-4">{formatTime(i.expiresAt)}</td>
								<td className="px-2 py-4">
									{i.acceptedAt ? formatTime(i.acceptedAt) : <span>-</span>}
								</td>
