...
export const PlayerVolumeSlider: FC<PlayerVolumeSliderProps> = ({
	audioAmplifier,
}) => {
	...
	return (
		<div className="flex items-center gap-2 w-40 sm:w-full sm:px-0">
			...
			<Slider.Root
				className="relative flex items-center cursor-pointer h-2 w-full"
				value={[volume]}
				max={300}
				onValueChange={(v) => {
			...
			<span className="inline-block text-xs text-(--fg-color) w-16">
				{volume}%
			</span>
		</div>
	)
