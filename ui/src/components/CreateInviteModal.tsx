...
export const CreateInviteModal = () => {
	...
	const [invite, setInvite] = useState<
		components['schemas']['InvitePostModel']
	>({ role: 'user', explicitConsent: false })
	const { t } = useTranslation()
	const setCreateInviteModalOpen = useCommon(
	...
	return createPortal(
		<div
			...
			className={`grid place-items-center fixed inset-0 bg-[rgba(0,0,0,0.5)] backdrop-blur-sm overflow-x-hidden overflow-y-auto z-30 ${inviteModalOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
			tabIndex={-1}
		>
			{/* Modal */}
			<div
				className="relative bg-(--bg-color) max-w-5xl p-8 rounded-2xl shadow-[0_4px_16px_rgba(0,0,0,0.2)]"
			...
			>
				...
				<div className="mb-6">
					<label
						className="block mb-2 text-sm text-(--fg-color)"
						htmlFor="role"
					>
						{t('role')}
					</label>
					<CustomSelect
						...
						onChange={(v) => {
							setInvite({
								...invite,
								role: v as components['schemas']['InvitePostModel']['role'],
							})
						}}
						options={roleOptions}
				...
				<div className="flex items-center gap-4 mb-6">
					<label
						className="text-sm text-(--fg-color)"
						htmlFor="allow-explicit-content"
					>
						{t('allow-explicit-content')}
					</label>
					<Switcher
						checked={invite.explicitConsent}
						id="allow-explicit-content"
						onChange={() => {
							setInvite({ ...invite, explicitConsent: !invite.explicitConsent })
						}}
					/>
				</div>
