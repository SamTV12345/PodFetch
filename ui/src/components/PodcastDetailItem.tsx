...
export const PodcastDetailItem: FC<PodcastDetailItemProps> = ({
	episode,
	index,
	currentEpisodes,
	onlyUnplayed,
	...
	const setSelectedEpisode = useAudioPlayer(
		...
		<Fragment key={'episode_' + episode.podcastEpisode.id}>
			...
			>
				...
				<img
					...
					alt={episode.podcastEpisode.name}
					className="
                    hidden xs:block
				...
				>
					<span className="text-sm text-(--fg-secondary-color)">
						{formatTime(episode.podcastEpisode.date_of_recording)}
					</span>
					<span className="text-sm text-(--fg-secondary-color)">
						{playedTime}
					</span>
					...
					<span className="flex gap-5">
						<span
							title={t('download-to-server') as string}
							className={`material-symbols-outlined text-(--fg-icon-color)
                     ${episode.podcastEpisode.status ? 'cursor-auto filled' : 'cursor-pointer hover:text-(--fg-icon-color-hover)'}`}
							onClick={(e) => {
								...
								client
									.PUT('/api/v1/podcasts/{id}/episodes/download', {
										params: {
											path: {
									...
									.then(() => {
										enqueueSnackbar(t('episode-downloaded-to-server'), {
											variant: 'success',
										})
						...
						{/* Check icon */}
						<span
							className="material-symbols-outlined text-(--fg-icon-color) active:scale-95"
							onClick={(e) => {
								// Prevent icon click from triggering info modal
								e.stopPropagation()
								logCurrentPlaybackTime(
									episode.podcastEpisode.episode_id,
									episode.podcastHistoryItem?.total || 0,
								)
								const mappedEpisodes = selectedEpisodes.map((s) => {
									if (
										s.podcastEpisode.episode_id ===
										episode.podcastEpisode.episode_id
									) {
										if (s.podcastHistoryItem) {
											s.podcastHistoryItem.position =
												episode.podcastEpisode.total_time
										} else {
						...
						<span
							className={
								'material-symbols-outlined text-(--fg-color) ' +
								(episode.podcastEpisode.favored && 'filled')
							}
							onClick={async () => {
								...
								for (const cache of queryClient.getQueryCache().getAll()) {
									if (
										...
										(
											cache.queryKey[2] as {
												params: operations['find_all_podcast_episodes_of_podcast']['parameters']
											}
										...
										queryClient.setQueryData(
											...
											(
												...
												return oldData.map((s) => {
													if (
														s.podcastEpisode.id === episode.podcastEpisode.id
													) {
														return {
															...s,
															podcastEpisode: {
				...
				{/* Play button */}
				<span
					className={`${percentagePlayed >= 95 && episode.podcastEpisode.total_time > 0 && 'text-gray-500'}
