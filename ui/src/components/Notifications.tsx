				...
				return (
					<span
						dangerouslySetInnerHTML={removeHTML(
							t('notification.episode-now-available', {
								episode: notification.message,
							}),
						)}
					/>
				)
...
export const Notifications: FC = () => {
	...
	const trigger = () => (
		<div className="flex items-center relative">
			<span className="material-symbols-outlined cursor-pointer text-(--fg-color) hover:text-(--fg-color-hover)">
				notifications
			</span>
			...
			{notifications.isLoading || !notifications.data ? (
				...
				notifications.data.length > 0 && (
					<div className="absolute top-0 right-0 border-2 border-(--bg-color) bg-red-700 h-3 w-3 rounded-full" />
				)
			)}
		</div>
	)
		...
		client
			...
			.then(() => {
				queryClient.setQueryData(
					...
					(oldData: components['schemas']['Notification'][]) => {
			...
			return (
				<AnimatePresence>
					{notifications.data.map((notification) => (
						<motion.div
							className="grid grid-cols-[1fr_auto] gap-2 last-of-type:border-b-0! border-b-(--border-color) px-5 text-sm text-(--fg-color)"
							key={notification.id}
							initial={false}
							animate={{
						...
						>
							...
							<span
								className="material-symbols-outlined cursor-pointer text-(--modal-close-color) hover:text-(--modal-close-color-hover)"
								onClick={() => {
