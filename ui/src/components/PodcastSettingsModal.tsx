...
import type { PodcastSetting } from '../models/PodcastSetting'
...
export const PodcastSettingsModal: FC<PodcastSettingsModalProps> = ({
	...
	const podcastSettings = $api.useQuery(
		'get',
	...
	const updatePodcastSettings = $api.useMutation(
		'put',
		'/api/v1/podcasts/{id}/settings',
	)
	...
	return (
		...
		>
			...
			<Dialog.Portal>
				<Dialog.Overlay
					className="fixed inset-0 grid place-items-center bg-[rgba(0,0,0,0.5)] backdrop-blur-sm
            overflow-y-auto overflow-x-hidden z-30 transition-opacity opacity-100"
				...
				<Dialog.Content className="fixed inset-0 z-40 flex items-center justify-center p-4">
					<div
						...
						className={
							'relative bg-(--bg-color) max-w-2xl p-8 rounded-2xl shadow-[0_4px_16px_rgba(0,0,0,var(--shadow-opacity))] '
						}
					>
						...
						<div
							className={`grid grid-cols-3 gap-5 ${!isSettingsEnabled && 'opacity-50'}`}
						>
							<label
								className="mr-6 text-(--fg-color) col-span-2"
							...
							<h2 className="text-(--fg-color) col-span-2">
								{t('episode-numbering')}
							</h2>
							<Switcher
							...
							/>
							<label className="mr-6 text-(--fg-color)" htmlFor="auto-cleanup">
								{t('auto-cleanup')}
							</label>
							...
							<Switcher
								...
								checked={
									podcastSettings.data
										? podcastSettings.data.autoCleanup
										: false
								}
							...
							/>
							<label
								htmlFor="days-to-keep"
								className="flex gap-1 col-span-2 text-(--fg-color)"
							>
								{t('days-to-keep')}
								<SettingsInfoIcon
							...
							<CustomInput
								...
								value={
									podcastSettings.data
										? podcastSettings.data?.autoCleanupDays
										: '0'
								}
							/>
							...
							>
								{t('auto-update')}{' '}
								<SettingsInfoIcon
									headerKey="auto-update"
									textKey="auto-update-explanation"
								/>
							</label>
							<Switcher
								disabled={!isSettingsEnabled}
								checked={
									podcastSettings.data ? podcastSettings.data.autoUpdate : false
								}
								className="xs:justify-self-end"
								id="auto-update"
								onChange={() => {
									queryClient.setQueryData(
										...
										(oldData: PodcastSetting | undefined) => {
											return {
												...oldData,
												autoUpdate: !podcastSettings?.data?.autoUpdate,
											}
										},
									)
								}}
							...
							<Switcher
								...
								checked={
									podcastSettings.data
										? podcastSettings.data.autoDownload
										: false
								}
								className="xs:justify-self-end"
								id="auto-download"
								onChange={() => {
									queryClient.setQueryData(
										...
										(oldData: PodcastSetting | undefined) => {
											return {
												...oldData,
												autoDownload: !podcastSettings?.data?.autoDownload,
											}
										},
									)
								}}
							/>
							<label
								className="text-(--fg-color) flex gap-1 col-span-2"
							...
							/>
							<label
								htmlFor="number-of-podcasts-to-download"
								className="flex gap-1 col-span-2 text-(--fg-color)"
							>
							...
							<CustomInput
								disabled={!isSettingsEnabled}
								className="w-20 justify-self-end"
								id="number-of-podcasts-to-download"
								onChange={(e) => {
									queryClient.setQueryData(
										[
											'get',
								...
								type="number"
								value={
									podcastSettings.data ? podcastSettings.data.podcastPrefill : 5
								}
							/>
							<label
								htmlFor="activate"
							...
							<Switcher
								checked={
									podcastSettings.data ? podcastSettings.data.activated : false
								}
								className="xs:justify-self-end"
								id="activate"
								onChange={() => {
