	...
	useEffect(() => {
		...
		socket.on('offlineAvailable', (data) => {
			...
			if (useCommon.getState().currentDetailedPodcastId === data.podcast.id) {
				enqueueSnackbar(
					t('new-podcast-episode-added', {
						name: decodeHTMLEntities(data.podcast_episode.name),
					}),
					{ variant: 'success' },
				)
				...
				const podcastUpdated = useCommon
					.getState()
					.selectedEpisodes.map((p) => {
						if (p.podcastEpisode.id === downloadedPodcastEpisode.id) {
							const foundDownload = JSON.parse(
								JSON.stringify(p),
							) as components['schemas']['PodcastEpisodeWithHistory']

							foundDownload.podcastEpisode.status = true
							foundDownload.podcastEpisode.url = downloadedPodcastEpisode.url
							foundDownload.podcastEpisode.local_url =
								downloadedPodcastEpisode.local_url
							foundDownload.podcastEpisode.image_url =
								downloadedPodcastEpisode.image_url
							foundDownload.podcastEpisode.local_image_url =
								downloadedPodcastEpisode.local_image_url
		...
		socket.on('opmlError', (data) => {
			...
			useOpmlImport
				.getState()
				.setMessages([...useOpmlImport.getState().messages, data.message])
		})
		...
		socket.on('refreshedPodcast', (data) => {
			...
			enqueueSnackbar(
				t('podcast-refreshed', { name: decodeHTMLEntities(podcast.name) }),
				{ variant: 'success' },
			)
		})
		...
		socket.on('addedEpisodes', (data) => {
			enqueueSnackbar(
				t('new-podcast-episode-added', {
					name: decodeHTMLEntities(data.podcast.name),
				}),
				{ variant: 'success' },
			)
		})
		...
		socket.on('addedPodcast', (data) => {
			...
			for (const cache of queryClient.getQueryCache().getAll()) {
				if (
					...
					(cache.queryKey[1] as string) === '/api/v1/podcasts/search'
				) {
			...
			enqueueSnackbar(
				t('new-podcast-added', { name: decodeHTMLEntities(podcast.name) }),
				{ variant: 'success' },
			)
		})

		socket.on('deletedPodcastEpisodeLocally', (data) => {
			const updatedPodcastEpisodes = useCommon
				.getState()
